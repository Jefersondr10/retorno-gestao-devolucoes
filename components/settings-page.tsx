      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:py-9">
        ...
        <main className="mt-6 min-w-0">
          {loading ? <Card className="grid min-h-48 place-items-center border-0 bg-card ring-border/80"><Loader2 className="size-6 animate-spin text-primary" /></Card> : <>
            ...
            {activeSection === 'usuarios' && <SettingsCard icon={<UsersRound />} title="Usuários e acessos" description="Crie acessos individuais, aprove solicitações feitas pelo Google, escolha permissões e encerre o acesso de quem não faz mais parte da operação."><UserManagementSection currentUser={currentUser} /></SettingsCard>}
          </>}
  ...
  return <div className={`${className} grid gap-2`}>{items.map((item) => {
    ...
    return <div key={item.code} className={`flex flex-col gap-3 rounded-2xl border p-3 sm:flex-row sm:items-center ${item.active ? 'bg-card' : 'bg-muted/35 opacity-75'}`}>
      <div className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-semibold">{item.label}</p><Badge variant={item.active ? 'secondary' : 'outline'}>{item.active ? 'Ativo' : 'Inativo'}</Badge>{item.is_system ? <Badge variant="outline">Padrão</Badge> : null}</div>{showPolicies && <p className="mt-1 text-xs font-medium text-muted-foreground">{item.requires_invoice ? 'Exige nota de entrada' : 'Dispensa nota'}{item.requires_notes ? ' · exige descrição' : ''}</p>}<p className="mt-1 text-xs text-muted-foreground">{Number(item.usage_count || 0)} devolução(ões) usando este cadastro</p></div>
      </div>
