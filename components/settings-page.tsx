...
export function SettingsPage() {
  ...
  async function saveEdit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    else { const option = [...options.locations, ...options.stores, ...options.conditions].find((item) => item.code === target.code); if (option) await updateOption(option, { label: target.label, color: target.color }); }
  }
  ...
  return (
    <div className="app-shell min-h-screen text-foreground">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:py-9">
        ...
        <div className="mt-6 lg:hidden"><Label htmlFor="settings-section" className="mb-2 block">Área de configuração</Label><NativeSelect id="settings-section" className="h-11 w-full bg-card" value={activeSection} onChange={(event) => setActiveSection(event.target.value as SettingsSection)}><NativeSelectOption value="status">Status e cores</NativeSelectOption><NativeSelectOption value="locais">Locais</NativeSelectOption><NativeSelectOption value="lojas">Lojas e cores</NativeSelectOption><NativeSelectOption value="condicoes">Condições</NativeSelectOption><NativeSelectOption value="retencao">Fotos e retenção</NativeSelectOption></NativeSelect></div>
      ...
      <Dialog open={Boolean(editTarget)} onOpenChange={(open) => !open && setEditTarget(null)}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Editar {editTarget?.kind === 'STATUS' ? 'status' : 'cadastro'}</DialogTitle><DialogDescription>A alteração será usada nas próximas seleções.</DialogDescription></DialogHeader>{editTarget && <form onSubmit={saveEdit} className="space-y-4"><LabeledInput id="edit-name" label="Nome" value={editTarget.label} onChange={(label) => setEditTarget((current) => current ? { ...current, label } : current)} placeholder="Nome do cadastro" />{editTarget.color && <ColorField id="edit-color" label="Cor" value={editTarget.color} onChange={(color) => setEditTarget((current) => current ? { ...current, color } : current)} />}<DialogFooter className="mx-0 mb-0 rounded-b-xl px-0 pb-0"><Button type="button" variant="outline" onClick={() => setEditTarget(null)}>Cancelar</Button><Button type="submit"><Save /> Salvar alteração</Button></DialogFooter></form>}</DialogContent></Dialog>
...
function ManagedOptions({ items, busy, empty = 'Nenhum cadastro encontrado.', className = '', showColor = false, onEdit, onToggle, onDelete }: { items: ConfigOption[]; busy: string; empty?: string; className?: string; showColor?: boolean; onEdit: (item: ConfigOption) => void; onToggle: (item: ConfigOption) => void; onDelete: (item: ConfigOption) => void }) {
  ...
  return <div className={`${className} grid gap-2`}>{items.map((item) => {
    ...
    return <div key={item.code} className={`flex flex-col gap-3 rounded-2xl border p-3 sm:flex-row sm:items-center ${item.active ? 'bg-card' : 'bg-muted/35 opacity-75'}`}>
      <div className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-semibold">{item.label}</p><Badge variant={item.active ? 'secondary' : 'outline'}>{item.active ? 'Ativo' : 'Inativo'}</Badge>{item.is_system ? <Badge variant="outline">Padrão</Badge> : null}</div><p className="mt-1 text-xs text-muted-foreground">{Number(item.usage_count || 0)} devolução(ões) usando este cadastro</p></div>
      </div>
