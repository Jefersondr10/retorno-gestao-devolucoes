  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 border-b border-border/80 bg-background/92 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-4 px-4 lg:px-7">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button size="lg" className="h-11 rounded-xl px-4 shadow-[0_8px_20px_rgb(13_96_83/18%)]" onClick={() => setCreateOpen(true)}><Camera /><span className="hidden sm:inline">Registrar recebimento</span><span className="sm:hidden">Registrar</span></Button>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-[1500px] grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="sticky top-16 hidden h-[calc(100vh-4rem)] border-r border-border/70 px-3 py-5 lg:block">
          <nav aria-label="Navegação principal" className="space-y-1">
            ...
            <NavButton icon={<Settings />} label="Configurações" onClick={() => setSettingsOpen(true)} />
          </nav>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <section className="mt-7">
              ...
              {error ? (
                ...
                <EmptyState hasFilters={Boolean(search || statusFilter)} onCreate={() => setCreateOpen(true)} onClear={() => { setSearch(''); setStatusFilter(''); }} />
              ) : (
      ...
      <nav aria-label="Navegação móvel" className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-4 rounded-2xl border border-border/80 bg-card/95 px-2 py-2 shadow-[0_14px_50px_rgb(25_35_32/18%)] backdrop-blur-xl lg:hidden">
        ...
        <MobileNav icon={<Camera />} label="Nova" onClick={() => setCreateOpen(true)} />
        ...
        <MobileNav icon={<Settings />} label="Mais" onClick={() => setSettingsOpen(true)} />
      </nav>
      ...
      <CreateReturnDialog open={createOpen} onOpenChange={setCreateOpen} onCreated={(item) => refresh(`${item.protocol} registrada com sucesso.`)} onView={setDetailId} />
      ...
      <StatusSettingsDialog open={settingsOpen} statuses={statuses} onOpenChange={setSettingsOpen} onStatusCreated={(status) => { setStatuses((current) => [...current, status]); setNotice(`Status “${status.label}” cadastrado.`); }} />
    </div>
  );
}
